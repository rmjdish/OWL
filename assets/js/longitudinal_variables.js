document.addEventListener("DOMContentLoaded", () => {
  ...
  const DICT_URL     = `${SITE_BASEURL}/assets/data/search_methods/data_dictionary/NSHD_Data_Dictionary_Public.json`;
  ...
  // Look up an individual variable's own label (not the group's shortened
  // Field-ID-level label). Falls back to the group label only if a sweep
  // entry can't be found, which shouldn't normally happen.
  function getVarLabel(field, varname) {
    const sweep = field.sweeps.find(s => s.varname === varname);
    return (sweep && sweep.label) ? sweep.label : field.label;
  }
  ...
  function renderTable() {
    ...
    tbody.querySelectorAll('.row-check').forEach(cb => {
      cb.onchange = () => {
        ...
        const fid   = parseInt(cb.dataset.fieldId);
        const field = allFields.find(f => f.fieldId === fid);
      ...
      const tableRows = sweepData.map((s, i) => {
        ...
        const checkCell = `<td class="sweep-add-col" style="border-bottom:1px solid #C4ADE8;border-right:1px solid #C4ADE8;">
          <input type="checkbox" class="sweep-check"
                 ...
                 data-label="${(s.label || field.label).replace(/"/g,'&quot;')}"
                 data-fid="${fid}"
                 ${inBasket ? 'checked' : ''}>
        </td>`;
