...
(function () {
  ...
  function dataBaseUrl() {
    // Where {varname}.json files live - defaults to the same directory
    // this script itself was loaded from, so variable.html and every
    // per-variable stub page can share one relative convention. Override
    // by setting window.OWL_DATA_BASEURL before this script loads.
    if (window.OWL_DATA_BASEURL) return window.OWL_DATA_BASEURL;
    var scripts = document.getElementsByTagName("script");
    for (var i = 0; i < scripts.length; i++) {
      var src = scripts[i].src || "";
      if (src.indexOf("variable_page.js") !== -1) {
  ...
  function drawContinuousChart(canvas, chart, label) {
    ...
    var padL = 46, padR = 20, padT = 56, padBottom = 40;
    var boxH = 46, gapBoxHist = 14;
    ...
    ["min", "max"].forEach(function (k) {
      ctx.beginPath();
      ctx.moveTo(xPos(box[k]), midY - 8); ctx.lineTo(xPos(box[k]), midY + 8);
      ctx.stroke();
    });
    var bx = xPos(box.q1), bw = Math.max(xPos(box.q3) - xPos(box.q1), 1);
    ctx.fillStyle = "rgba(156,135,214,0.55)";
    ctx.fillRect(bx, midY - 16, bw, 32);
    ctx.strokeStyle = PURPLE.edge; ctx.strokeRect(bx, midY - 16, bw, 32);
    ctx.beginPath();
    ctx.moveTo(xPos(box.median), midY - 16); ctx.lineTo(xPos(box.median), midY + 16);
    ...
    ctx.fillStyle = "#555"; ctx.font = "8px system-ui, sans-serif"; ctx.textAlign = "center";
    [xMin, (xMin + xMax) / 2, xMax].forEach(function (v) {
      ctx.fillText(round4(v), xPos(v), histTop + histH + 14);
    });
    ctx.save();
    ctx.translate(14, histTop + histH / 2);
  ...
  function drawCategoricalChart(canvas, chart) {
    ...
    var dpr = window.devicePixelRatio || 1;
    var rowH = 26, chartH = Math.max(n * rowH + 90, 200);
    var W = 760, H = chartH;
    canvas.width = W * dpr; canvas.height = H * dpr;
    canvas.style.width = W + "px"; canvas.style.height = H + "px";
    var ctx = canvas.getContext("2d");
    ...
    bars.forEach(function (b, i) {
      ...
      roundRect(ctx, padL, y, Math.max(w, 2), rowH - 8, 6);
      ctx.stroke();
      ctx.fillStyle = PURPLE.edge; ctx.textAlign = "left";
    ...
    var stackY = padT + n * rowH + 20, stackH = 22;
  ...
  function renderDist(page, varname) {
    if (page.is_genomic || page.is_plot_excluded) {
      return (
        '<div class="vm-panel" data-panel="dist"><p style="font-size:14px;color:#555;font-style:italic;">' +
        "Plots and statistics for this variable are not displayed.</p></div>"
      );
    }
    if (!page.chart && (!page.freq_rows || !page.freq_rows.length)) {
      return '<div class="vm-panel" data-panel="dist"><p style="color:#666;font-style:italic;">No distribution data available.</p></div>';
    }
