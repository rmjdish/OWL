document.addEventListener("DOMContentLoaded", () => {
  ...
  Promise.all([
    ...
    fetch("/OWL/assest/data/search_methods/data_dictionary/NSHD_Data_Dictionary_Public.json").then(r => r.json())
  ])
  .then(([popular, labels]) => {
  ...
  // the year-chip filter offers — nothing is hardcoded.
  ...
  // ⭐ Turn raw per-year records into the flat rows the table needs.
  // Displayed count = sum across whichever years are selected.
  // Qualification = count > MIN_PER_YEAR in at least one selected year.
  function buildRows(popular, labelMap, fieldIdMap, dictionaryNames, selectedYears) {
    return popular
      .map(row => {
        const count = [...selectedYears].reduce(
          (sum, y) => sum + (row.counts && row.counts[y] ? row.counts[y] : 0),
          0
        );
        const qualifies = [...selectedYears].some(
          y => (row.counts && row.counts[y] ? row.counts[y] : 0) > MIN_PER_YEAR
        );
        return {
          name:    row.name,
  ...
  function initUI(popular, labelMap, fieldIdMap, dictionaryNames, availableYears, selectedYears) {
    ...
    let data         = buildRows(popular, labelMap, fieldIdMap, dictionaryNames, selectedYears);
    let filteredData = [...data];

    // ⭐ YEAR CHIPS — built from availableYears, not a hardcoded list
    function renderYearChips() {
      if (!yearChipsEl) return;

      const allSelected = availableYears.length > 0 &&
        availableYears.every(y => selectedYears.has(y));

      const chipsHtml = [
        `<button type="button" class="year-chip${allSelected ? " active" : ""}" data-year="all">All years</button>`,
        ...availableYears.map(y =>
          `<button type="button" class="year-chip${selectedYears.has(y) ? " active" : ""}" data-year="${y}">${y}</button>`
        )
      ].join("");

      yearChipsEl.innerHTML = chipsHtml;

      yearChipsEl.querySelectorAll(".year-chip").forEach(btn => {
        btn.onclick = () => {
    ...
    function onYearsChanged() {
      data = buildRows(popular, labelMap, fieldIdMap, dictionaryNames, selectedYears);
      applySearch();
      currentPage = 1;
      renderYearChips();
