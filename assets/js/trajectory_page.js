...
(function () {
  ...
  // domain: [min, max] SHARED across every card in the group, so every
  // card's x-axis ticks land at the same pixel position - that shared
  // scale is what "aligned" means here, not just visual similarity.
  function drawContinuousMini(canvas, chart, domain, label) {
  ...
  // categories: the FULL, ordered union of distinct category values seen
  // across the whole group (same order, same count-axis max, on every
  // card) - the categorical equivalent of the shared numeric domain above.
  function drawCategoricalMini(canvas, chart, categories, maxCount) {
    ...
    var plotW = W - padL - padR;
    var byValue = {};
    chart.bars.forEach(function (b) { byValue[String(b.value)] = b; });

    ctx.font = "10px system-ui, sans-serif";
    categories.forEach(function (catVal, i) {
      var y = padT + i * rowH;
      var b = byValue[catVal];
      var w = b ? (b.count / maxCount) * plotW : 0;
      ctx.fillStyle = shadeFor(i, n);
      ctx.fillRect(padL, y, Math.max(w, b ? 2 : 0), rowH - 9);
      if (b) {
      ...
      ctx.fillText(b ? String(b.count) : "\u2014", padL + w + 5, y + (rowH - 9) / 2 + 3);
      ctx.textAlign = "right";
      var lbl = (b && b.label) || catVal;
  ...
  function render(via, members, results) {
    ...
    // Shared domain (continuous) / shared category order + max (categorical)
    var domain = null, categories = null, catMax = 1;
    if (refType === "continuous") {
      var mins = [], maxs = [];
      ok.forEach(function (r) {
        if (r.data.page.dist_type === "continuous" && r.data.page.chart) {
          var e = r.data.page.chart.bins.edges;
          mins.push(e[0]); maxs.push(e[e.length - 1]);
        }
      });
      domain = [Math.min.apply(null, mins), Math.max.apply(null, maxs)];
    } else if (refType === "categorical") {
      var seen = {}; var order = [];
      ok.forEach(function (r) {
      ...
      ok.forEach(function (r) {
        if (r.data.page.dist_type === "categorical" && r.data.page.chart) {
          r.data.page.chart.bars.forEach(function (b) { catMax = Math.max(catMax, b.count); });
        }
      });
    }
    ...
    var cardsHtml = members.map(function (m, idx) {
      ...
      if (pg.dist_type === "unavailable" || pg.is_plot_excluded || pg.is_genomic || !pg.chart) {
        return '<div class="traj-card">' + headHtml + '<div class="traj-card-placeholder">Plots/statistics unavailable for this sweep</div></div>';
      }
      var canvasId = "traj-canvas-" + idx;
      return '<div class="traj-card">' + headHtml + '<canvas id="' + canvasId + '"></canvas></div>';
    }).join("");

    var statsHtml = buildStatsTable(refType, members, ok, categories);

    app.innerHTML =
      nav + '<div class="traj-wrapper">' + head +
    ...
    setTimeout(function () {
      members.forEach(function (m, idx) {
        ...
        if (!canvas) return;
        if (pg.dist_type === "continuous") drawContinuousMini(canvas, pg.chart, domain, pg.label);
        else if (pg.dist_type === "categorical") drawCategoricalMini(canvas, pg.chart, categories, catMax);
      });
